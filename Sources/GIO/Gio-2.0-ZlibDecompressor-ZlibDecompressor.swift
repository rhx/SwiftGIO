...
open class ZlibDecompressor: Object, ZlibDecompressorProtocol {
    /// Designated initialiser from the underlying `C` data type.
    /// Ownership is transferred to the `ZlibDecompressor` instance.
    public init(_ op: UnsafeMutablePointer<GZlibDecompressor>) {
        super.init(cast(op))
    }

    /// Reference convenience intialiser for a related type that implements `ZlibDecompressorProtocol`
    /// Will retain `GZlibDecompressor`.
    public convenience init<T: ZlibDecompressorProtocol>(_ other: T) {
        self.init(cast(other.zlib_decompressor_ptr))
        g_object_ref(cast(zlib_decompressor_ptr))
    }

    /// Unsafe typed initialiser.
    /// **Do not use unless you know the underlying data type the pointer points to conforms to `ZlibDecompressorProtocol`.**
    public convenience init<T>(cPointer: UnsafeMutablePointer<T>) {
        self.init(cPointer.withMemoryRebound(to: GZlibDecompressor.self, capacity: 1) { $0 })
    }

    /// Unsafe untyped initialiser.
    /// **Do not use unless you know the underlying data type the pointer points to conforms to `ZlibDecompressorProtocol`.**
    public convenience init(raw: UnsafeRawPointer) {
        self.init(UnsafeMutableRawPointer(mutating: raw).assumingMemoryBound(to: GZlibDecompressor.self))
    }

    /// Unsafe untyped initialiser.
    /// **Do not use unless you know the underlying data type the pointer points to conforms to `ZlibDecompressorProtocol`.**
    public convenience init(raw: UnsafeMutableRawPointer) {
        self.init(raw.assumingMemoryBound(to: GZlibDecompressor.self))
    }

    /// Unsafe untyped initialiser.
    /// **Do not use unless you know the underlying data type the pointer points to conforms to `ZlibDecompressorProtocol`.**
    public convenience init(opaquePointer: OpaquePointer) {
        self.init(UnsafeMutablePointer<GZlibDecompressor>(opaquePointer))
    }

    /// Creates a new `GZlibDecompressor`.
    public convenience init( format: ZlibCompressorFormat) {
        let rv = g_zlib_decompressor_new(format)
        self.init(cast(rv))
    }
